...
def test_minio_hostname_patch_redirects_str_and_bytes():
    from pipeline.client import _patched_getaddrinfo
    ...
    import pipeline.client as client_module

    original = client_module._real_getaddrinfo
    client_module._real_getaddrinfo = fake_getaddrinfo
    try:
        assert _patched_getaddrinfo("minio", 9000) == "sentinel"
        assert called_with["host"] == "localhost"
        assert _patched_getaddrinfo(b"minio", 9000) == "sentinel"
        assert called_with["host"] == "localhost"
        assert _patched_getaddrinfo("example.com", 80) == "sentinel"
