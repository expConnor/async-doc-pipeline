...
### Application-generated UUIDs and opaque object keys

Primary keys (`documents.id`, `jobs.id`, `artifacts.id`) are UUIDs generated by the application before the row is inserted, not DB-assigned auto-increment integers. The API needs the `document_id` at the moment it builds the S3 object key for the presigned upload URL — before any row exists to auto-generate one from. Generating the UUID in application code sidesteps that ordering problem entirely.

Object keys are derived solely from `account_id` and `document_id` — `raw/{account_id}/{document_id}.pdf` and `artifacts/{account_id}/{document_id}.md` — with no filename, job id, or other user-supplied text embedded. This keeps keys deterministic — the same document always maps to the same key, which is what lets reprocessing overwrite the existing artifact object and row in place instead of accumulating duplicates — and avoids ever putting untrusted client input into an S3 path.
...
```
1. POST /documents (no request body)
   → validates API key
   → generates a UUID document_id application-side
   → creates Document record (stores object key raw/{account_id}/{document_id}.pdf, not the file)
   → returns document_id and a presigned S3 PUT URL
...
4. Worker receives message
   ...
   → uploads Markdown to S3 as artifacts/{account_id}/{document_id}.md
